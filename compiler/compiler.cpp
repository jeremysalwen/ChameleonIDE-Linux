...
Compiler::Compiler(Options* options, Networking* network)
{
	...
	m_mingwPath = wxT("d:\\toolkits\\MinGW513\\");
}
...
void Compiler::StartNextFile()
{
	...
	wxFileName outFile(inFile.GetPath(wxPATH_GET_VOLUME, isRemote ? wxPATH_UNIX : wxPATH_DOS) + wxT("/") + inFile.GetName() + wxT(".o"));
	...
	if(isRemote) {
		...
		cmd +=  wxT(" && echo C_O_M_P_I_L_E_SUCCESS || echo C_O_M_P_I_L_E_FAILED");
	}
	else {
		...
		wxFileName compilerPath = executableNames[wxT("g++.exe")];

		//cmd +=  m_mingwPath + "/bin/g++.exe\" "; // compiler
		compileCommand += wxT("\"");
		compileCommand += compilerPath.GetFullPath();
		compileCommand +=  wxT("\" -g -c -fmessage-length=0 ");
		compileCommand +=  wxT(" -o ") + outFile.GetFullPath(wxPATH_DOS) + wxT(" ");
...
void Compiler::StartLinking() {
	...
	{
		//name += "/" + name + ".out";
		outputFile.SetExt(wxT("out"));
	}
	else
	{
		//name += "\\" + name + ".exe";
		outputFile.SetExt(wxT("exe"));
	}

	// Assemble inFiles list:
	wxString inFiles = wxEmptyString;
	for(unsigned int i = 0; i < m_intermediateFiles.Count(); i++) {
		inFiles += wxT(" ") + m_intermediateFiles.Item(i);
	}
	for(unsigned int i = 0; i < m_currProj->GetLibraries().Count(); i++) {
		inFiles += wxT(" ") + m_currProj->GetLibraries().Item(i);
	}

	// Start the linking:
	wxString cmd = wxEmptyString;
	if(isRemote) {
		cmd += wxT("g++ -g -fmessage-length=0 -o ") + outputFile.GetFullPath(wxPATH_UNIX);
		cmd +=  inFiles;
		cmd +=  wxT(" && echo C_O_M_P_I_L_E_SUCCESS || echo C_O_M_P_I_L_E_FAILED");
	}
	else {
		...
		linkCommand +=  wxT("\" -g -fmessage-length=0 -o ") + outputFile.GetFullPath(wxPATH_DOS);
		linkCommand +=  inFiles;
		//cmd +=  " && echo C_O_M_P_I_L_E_SUCCESS || echo C_O_M_P_I_L_E_FAILED";
	...
	CompilerEvent e(chEVT_COMPILER_START);
	e.SetRemoteFile(isRemote);
	e.SetFile(wxFileName(wxT("Linking"))); // kind of icky
	AddPendingEvent(e);
...
void Compiler::RemoveIntermediateFiles() {
	...
	for(unsigned int i = 0; i < m_intermediateFiles.Count(); i++) {
		files += wxT(" ") + m_intermediateFiles.Item(i);
	}

	if(m_currProj->IsRemote()) {
		m_network->StartRemoteCommand(wxT("rm ") + files, this); //pehaps NULL (instead of this)
	}
	else {

		wxString deleteCommand = wxT("del ") + files;
...
void Compiler::OnProcessTerm(ChameleonProcessEvent& e)
{
	...
	if(m_isCompiling) {
		...
		wxLogDebug(wxT("Compiler received extraneous term event: %s"), e.GetString().wc_str());
...
{
	if(m_isCompiling) {
		m_compilerStdIn->WriteString(wxString()+(char)3); // send ^C -- break
...
void Compiler::OnProcessOut(ChameleonProcessEvent& e)
{
	...
	if(m_isCompiling || m_isLinking) {
		...
		if(s.Contains(wxT("C_O_M_P_I_L_E_SUCCESS"))) {
			s.Remove(s.Find(wxT("C_O_M_P_I_L_E_SUCCESS")));
		}
		else if(s.Contains(wxT("C_O_M_P_I_L_E_FAILED"))) {
			s.Remove(s.Find(wxT("C_O_M_P_I_L_E_FAILED")));
		...
		wxLogDebug(wxT("Compiler received output while not compiling: ") + e.GetString());
	}
}
...
{
	wxString cmd = wxT("cmd /c");
	cmd += wxT(" set PATH=%PATH%;"); //D:\\toolkits\\MinGW513\\libexec\\gcc\\mingw32\\3.4.2;D:\\toolkits\\MinGW513\\bin; && ";

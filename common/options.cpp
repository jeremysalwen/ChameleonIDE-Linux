...
Options::Options()
{
	...
	m_pscpProg = wxT("pscp.exe");
	m_plinkProg = wxT("plink.exe");
	//m_mingwPath = "C:\\Program Files\\MinGW\\bin\\";
	m_username = wxT("username");
	m_hostname = wxT("localhost");
	m_password = wxT("");
	m_remoteCompileOut = wxT("a.out");
	m_terminalSize = 100;
	...
	m_mingwProgramNames.Add(wxT("g++.exe"));
	m_mingwProgramNames.Add(wxT("cc1plus.exe"));
	m_mingwProgramNames.Add(wxT("gdb.exe"));
}
...
bool Options::SetPlinkApp(wxString path_and_prog) {
	...
	//else
		wxLogDebug(wxT("\"") + path_and_prog + wxT("\" is an invalid Plink."));
		return false;
}
...
wxString Options::VerifyMingwPath(wxString mingwPath)
{
	...
	{
		errorMessage = wxT("The directory does not exist.");
		goto Finished;
	}



	for(unsigned int i = 0; i < mingwFiles.GetCount(); i++)
	{
		...
		{
			errorMessage.Printf(wxString(wxT("Unable to find file: %s")).wc_str(), programName.wc_str());
			goto Finished;
		}
		else
		{

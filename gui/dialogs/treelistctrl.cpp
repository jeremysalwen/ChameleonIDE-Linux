...
wxString MakeShortString(wxDC& dc, const wxString&StringToModify, int nColumnLen, int nOffset)
{
	...
	static char* szShort=NULL;
	delete[] szShort;

    szShort=new char[nStringLen+strlen(szThreeDots)+1];
	...
	strncat(szShort, szThreeDots,nStringLen+strlen(szThreeDots));
	return szShort;
}
    ...
    switch (event.GetKeyCode()) {
        ...
        default:
            if (event.GetKeyCode() >= (int)' ') {
                ...
                m_findStr.Append (wxString((char)event.GetKeyCode()));

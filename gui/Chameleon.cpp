       ...
       : wxFrame((wxFrame *)NULL, -1, title, pos, size)
{
	...
	m_updateTimer = NULL;
/* Removed for linux
	const wxIcon icon(wxICON(IDI_APPICON));
...
void ChameleonWindow::SaveProjectFile()
{
	...
	{
		outputStream.CopyTo(wxStringBuffer(resultContents,streamsize),streamsize);
	}
...
void ChameleonWindow::OnCopy()
{
	...
	*/
	...
	if(focusedWindow == m_terminal || focusedWindow == m_debugTerminal)
	{
		...
		focusedTerminal->ProcessWindowEvent(ctrlCEvent);
	}
	else
	{
		m_currentEd->Copy();
